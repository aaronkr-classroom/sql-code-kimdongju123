...
2. 데이터웨어하우스, 데이터마트나 데이터마이닝에서는 수집된 데이터를 분석이 용이하도록 통일된 형태로 변환하는 과정을 (데이터정제 )이라고 한다.
...
스키마|	고정 스키마|	유연한 스키마|
쿼리 언어|	SQL 표준 언어|	SQL 및 NoSQL 언어 사용 가능|
유형|	관계형 데이터베이스|	키-값, 문서, 컬럼 패밀리, 그래프 데이터베이스 등|
사용 사례|	금융, ERP, 정형 데이터 관리|	빅데이터, IoT, 소셜 네트워크|
